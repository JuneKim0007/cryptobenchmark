...
class OverrideResolverTest {
    ...
    fun theNarrowestRuleWinsAndListsReplace() {
        ...
        val entry = resolver.resolve(gcm)
    ...
    fun providerDefaultsAreNamed() {
        assertEquals(listOf("keySize", "parameters"), OverrideResolver(emptyList()).resolve(gcm).providerDefaults)
        ...
        assertEquals(emptyList<String>(), explicit.resolve(gcm).providerDefaults)
    }
    ...
    fun anObservedSizeIsNeverTurnedIntoAnArgument() {
        ...
        val entry = OverrideResolver(emptyList()).resolve(desede)
    ...
    fun aKeySpecLeavesNoSize() {
        ...
        val entry = OverrideResolver(listOf(Override(Rule(name = "SHA256withECDSA"), key = curve))).resolve(ec)
        assertEquals(emptyList<Int>(), entry.keySizes)
