...
class OperationInputTest {
    ...
    private fun inputFor(case: BenchmarkCase, key: KeyMaterial, parameters: BoundParameters?) =
        OperationDefinitions.of(case.operation).input(case, key, parameters)
    ...
    fun messagesAreSeededAndASmallerSizeIsAPrefixOfALargerOne() {
        ...
        val message = inputFor(case("MessageDigest", "SHA-256", "SUN", Operation.DIGEST), KeyMaterial.None, null) as OperationInput.Message
    ...
    fun decryptGetsTheCiphertextAndTheSpecItWasMadeUnder() {
        ...
        val input = inputFor(case("Cipher", "AES/GCM/NoPadding", "SunJCE", Operation.DECRYPT), aes, gcm) as OperationInput.Ciphertext
    ...
    fun decryptWithoutASpecCarriesTheProvidersParameters() {
        val input = inputFor(case("Cipher", "AES/CBC/PKCS5Padding", "SunJCE", Operation.DECRYPT), aes, null) as OperationInput.Ciphertext
    ...
    fun asymmetricDecryptEncryptsWithThePublicKey() {
        val input = inputFor(case("Cipher", "RSA/ECB/OAEPPadding", "SunJCE", Operation.DECRYPT, inputSize = 32), rsa, null) as OperationInput.Ciphertext
    ...
    fun verifyGetsAValidSignature() {
        val input = inputFor(case("Signature", "SHA256withECDSA", "SunEC", Operation.VERIFY), ec, null) as OperationInput.SignedMessage
    ...
    fun keyOperationsTakeNoInput() {
        assertSame(OperationInput.None, inputFor(case("KeyPairGenerator", "RSA", "SunRsaSign", Operation.GENERATE_KEY_PAIR, inputSize = null), KeyMaterial.None, null))
    }
}
